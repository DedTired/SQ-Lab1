...
public class Binary
{
	...
	*/
	...
	{
		// the Index of first digit of each number
		int ind1=num1.number.length()-1;
		int ind2=num2.number.length()-1;
		...
		while(ind1>=0 || ind2>=0){// Will Loop until all digits are processed
			...
			// check for remianing digit
			if (ind1 >= 0){ // no digits left if less than 0
				temp = (num1.number.charAt(ind1) == '1')? 1:temp;
				ind1--;
			}
			if (ind2 >= 0){// no digits left if less than 0
				temp = (num2.number.charAt(ind2) == '1')? 1:temp;
				ind2--;
			}

			// store current digit calculation
